...
- 状態バッジは `pinned` と `transformed` のみ。見出し行に置く
- **要素によらず同じ内容が出続けるものはバッジにしない。** `cid …`（Astro のスコープ属性）と `unreadable: …`（読めなかったシート）はどちらも文書単位の事実なので、要素ごとのパネルに出すと常時点いている飾りにしかならない。前者はセレクタに既に現れており、後者を出すならインジケータ側（§F4）が居場所
...
ピン留めと hover の区別は**色ではなく線種**でつける（ピンは実線、hover は破線）。彩度を 1 色に絞ったぶん、区別は形に持たせる。
...
| オーバーレイの配色 | `#0000FF` 主体、ピンは `#000000`（§5） |
| ピン留めと hover の区別 | 色ではなく線種（ピン実線 / hover 破線）（§5） |
