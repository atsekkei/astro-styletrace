...
仕様は [spec.md](./spec.md)。現在の実装は **M1 〜 M5**（オーバーレイ / 距離計測 / 出自解決 / 3 列表示 / エディタジャンプ）。
...
パネルの出自ファイル名がボタンになっている。押すと dev server 経由で
`launch-editor` がエディタを起動する。エディタの選択は `LAUNCH_EDITOR` /
`EDITOR` 環境変数、または起動中のエディタからの推測に任せている。

CSSOM はルールの行番号を持たないので、**開けるのはファイル単位まで**（spec §6.9）。
行までは辿れないため、セレクタ横の `copy` でセレクタをコピーし、エディタ側で検索する。
cross-origin の CSS はディスク上に無いのでボタンにならない。
